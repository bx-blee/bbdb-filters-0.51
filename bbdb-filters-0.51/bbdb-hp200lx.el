...
;;;
;;;
;;; RCS: $Id: bbdb-hp200lx.el,v 1.1.1.1 2007/02/23 22:32:57 mohsen Exp $
;;;
...
;;;XXX;;; format, invoke M-x bbdb-output-hp200lx
;;;XXX;;;
;;;XXX;;; - you may also want to modify default values of the following (use
;;;XXX;;;   M-x describe-variable for details):
;;;XXX;;;     bbdb-output-hp200lx-elide
;;;XXX;;;     bbdb-output-hp200lx-requires
;;;XXX;;;     bbdb-output-hp200lx-no-bare-names
...
(defvar bbdb-output-hp200lx-elide '(net creation-date timestamp mail-alias)
...
See also variable bbdb-output-hp200lx-requires.")


(defvar bbdb-output-hp200lx-requires '(or name comp)
...
See also variable bbdb-output-hp200lx-elide.
")


(defvar bbdb-output-hp200lx-no-bare-names t
  "*A bare name is one with no information other than
that in bbdb-output-hp200lx-requires.  To avoid printing
these set this variable to t")


(defun bbdb-output-hp200lx (to-file)
...
Someday, optional third arg BRIEF will produce one-line format."
  ...
  (let* ((bbdb-elided-display bbdb-output-hp200lx-elide)
	 (first-letter
    ...
    (if (eval bbdb-output-hp200lx-requires)
	(let (more-phones)
	  ...
	  (if (and bare bbdb-output-hp200lx-no-bare-names)
