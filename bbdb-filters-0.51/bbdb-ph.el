...
;;;
;;;
;;; RCS: $Id: bbdb-ph.el,v 1.1.1.1 2007/02/23 22:32:57 mohsen Exp $
;;;
...
;;;XXX;;; format, invoke M-x bbdb-output-ph
;;;XXX;;;
;;;XXX;;; - you may also want to modify default values of the following (use
;;;XXX;;;   M-x describe-variable for details):
;;;XXX;;;     bbdb-output-ph-elide
;;;XXX;;;     bbdb-output-ph-requires
;;;XXX;;;     bbdb-output-ph-no-bare-names
...
(defvar bbdb-output-ph-elide '(creation-date timestamp mail-alias)
...
See also variable bbdb-output-ph-requires.")


(defvar bbdb-output-ph-requires '(and name net)
...
See also variable bbdb-output-ph-elide.
")


(defvar bbdb-output-ph-no-bare-names t
  "*A bare name is one with no information other than
that in bbdb-output-ph-requires.  To avoid printing
these set this variable to t")


(defun bbdb-output-ph (to-file)
...
Someday, optional third arg BRIEF will produce one-line format."
  ...
  (let* ((bbdb-elided-display bbdb-output-ph-elide)
	 (first-letter
    ...
    (if (eval bbdb-output-ph-requires)
	(progn
	  ...
	  (if (and bare bbdb-output-ph-no-bare-names)
