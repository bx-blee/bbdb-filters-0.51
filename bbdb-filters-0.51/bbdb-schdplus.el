...
;;;
;;;
;;; RCS: $Id: bbdb-schdplus.el,v 1.1.1.1 2007/02/23 22:32:57 mohsen Exp $
;;;
...
;;;XXX;; BBDB SCHDPLUS Filter
;;;XXX(load "bbdb-schdplus")

;;;XXX(setq bos-filename
;;;XXX      (concat "/dos/u/" (user-login-name) "/bbdb-schdplus.csv"))
;;;XXX;;; - to output the *BBDB* buffer in SCHDPLUS comma-delimited-file (.CSV)
;;;XXX;;; format, invoke M-x bbdb-output-schdplus

(defvar bos-filename "~/bbdb-schdplus.csv"
  "*Default file name for bbdb-output-schdplus printouts of BBDB database.")
...
(defvar bos-fields '(
		     ...
		     ("Business Street Address" (if business-address (bbdb-address-streets business-address)
									     ))
		     ...
		     ("Home Street Address" (if home-address (bbdb-address-streets home-address)
								     ) nil)
...
(defun bbdb-output-schdplus (to-file)
  "Print the selected BBDB entries in MS Schedule+ export file format."
  (interactive (list (read-file-name "Print To File: "
				     (file-name-directory bos-filename)
				     (file-name-nondirectory bos-filename)
				     nil
				     (file-name-nondirectory bos-filename)
				     )))
  (setq bos-filename (expand-file-name to-file))
  (let ((records (progn (set-buffer bbdb-buffer-name) bbdb-records)))
    (if records
	(let ((current-letter t)
	      bos-output-buffer)
	  (setq bos-output-buffer (find-file bos-filename))
	  (delete-region (point-min) (point-max))
	  (bos-do-prelude bos-output-buffer)
	  (while records
	    (setq current-letter
		  (bos-maybe-output-record (car (car records)) bos-output-buffer current-letter))
	    ;;(ding)
	    ;;(insert (format "ME ME ME"))
	    (setq records (cdr records)))
	  (bos-do-postlude bos-output-buffer)
	  (goto-char (point-min))
	  (save-buffer)
	  (message "Microsoft Schedule+ comma-delimited export file %s generated." bos-filename))
...
(defun bos-do-prelude (output-buffer)
...
(defun bos-maybe-output-record (record output-buffer &optional current-letter brief)
...
Someday, optional third arg BRIEF will produce one-line format."
  ...
  (let* ((first-letter (substring (concat (bbdb-record-sortkey record) "?") 0 1))
    ...
    (save-excursion
      (set-buffer (get-buffer-create " *bos-scratch*"))
	...
	(insert (format "%s: [%s|%s|%s|%s]\t"
			...
			(bbdb-address-streets (car more-addresses))
...
;	  (if (bbdb-field-shown-p (car thisnote))
		...
		(if (eq 'notes (car thisnote))
		    (insert (format "[notes] %s\t" (bos-mangle-if-multi-line (cdr thisnote))))
		  (insert (format "[%s] %s\t"
				  (car thisnote)
				  (bos-mangle-if-multi-line (cdr thisnote)))))))
;	)
    ...
    (progn
      (mapcar '(lambda (field-info)
		 (let (schdplus-field-name field-handler raw-string
					   (cooked-string ""))
		   (setq schdplus-field-name (nth 0 field-info))
		   ...
		   (cond ((stringp (setq raw-string (eval field-sexpr)))
			  (setq cooked-string
				(format "\"%s\""
					(cond (field-width (bos-maybe-truncate raw-string field-width))
					      (t raw-string))))
...
(defun bos-do-postlude (output-buffer)
  ;; noop
  )
...
(defun bos-insert-dos-eol()
  (insert "\n"))


(defun bos-maybe-truncate (string maxlen)
...
(defun bos-mangle-if-multi-line (string)
  "If STRING is has multiple lines, mangle it for output to SCHDPLUS"
  (if (string-match "\n" string)
      (string-replace-regexp string "\n" "\t") ; tabs are used to denote new lines in the .csv file
  string))

;;; ---------------------------------------------------------------------
(provide 'bbdb-schdplus)
